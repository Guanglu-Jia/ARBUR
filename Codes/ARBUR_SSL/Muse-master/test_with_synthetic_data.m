...
% no time delays, implies that the source is at the origin没有时间延迟，意味着源位于原炿
...
% delay the signals appropriately适当延迟信号??±?????%bsxfun对两个矩阵每个元素进行指定运算（由fun决定），并且具有自动扩维的作甿
rsubR=bsxfun(@minus,[r_true;0],R);  % 3 x n_mike, pos rel to each mike每个麦克风的位置,0减R承??元素
...
% multiple by the respective gain factors乘以各自的增益因孿
...
% add noise
sigma_v=0.020;  % V
noise=normrnd(0,sigma_v,n_t,n_mikes);%NORMRND正濥??布的随机数绿
