...
%% 计算每个信号的信号最强值
seg.max_intensity = [];
for i = 1:length(seg.data)
    seg.max_intensity(i,1) = max(max(seg.data{i}));
end
...
count = count+1;
source_estimated_all{count} = source_estimated;
