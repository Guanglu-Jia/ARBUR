...
% lower-numbered mike _after_ the higher-numbered mike.姝ts琛ㄧず澹伴煶鍒拌揪缂栧彿杈冧綆鐨勯害鍏嬮鍦ㄧ紪鍙疯緝楂樼殑楹﹀厠椋庝箣鍚庛?
% M=mixing_matrix_from_n_mics(K);

% Go to freq domain
V=fft(v);

% filter the signals in the freq domain鍦ㄩ鍩熶腑婊ゆ尝淇″彿
f=fft_base(N,1/(dt*N));
keep=(f_lo<=abs(f))&(abs(f)<f_hi);
V_filt=V;
V_filt(~keep,:)=0;
N_filt=sum(keep);  % this is the effective N for statistical purposes杩欐槸鐢ㄤ簬缁熻鐩殑鐨勬湁鏁圢
%tau=fftshift(fft_base(n_t,dt));  % want large neg times first鍏堣澶ц礋鏁?%v_filt=real(ifft(V_filt));
...
% (relative delay per pair) for each point in the grid计算网格中每个点的tau向量（每个麦克的延迟）和deltau向量%（每个对的相对延迟
[n_x,n_y]=size(x_grid);
n_r=n_x*n_y;
r_scan=zeros(3,1,n_r);%浜х敓x*y涓┖鍧愭爣
r_scan(1,:,:)=x_grid(:);
r_scan(2,:,:)=y_grid(:);
% z coords stay zeroz鍧愭爣淇濇寔涓洪浂z坐标保持为零
rsubR=bsxfun(@minus,r_scan,R);  % 3 x n_mike x n_r, pos rel to each mike.3 x n_mike x n_r锛屾瘡涓偣鍑忓幓姣忎釜mike瀵瑰簲鐨勪綅缃殑鍊?
d=reshape(sqrt(sum(rsubR.^2,1)),[K n_r]);  % m, 1 x n_mike x n_r
tau=(1/vel)*d;  % predicted time delays, s, n_mike x n_r棰勬祴鏃堕棿寤惰繜锛宻锛宯_mike x n_r
%delta_tau=M*tau;  % TDOA for each pair, s, n_pair x n_r

% compute the sum-of-squared-error at each grid point璁＄畻姣忎釜缃戞牸鐐圭殑骞虫柟璇樊涔嬪拰计算每个网格点的平方误差之和
