...
% calculate the time lag for each element of xcorr_raw计算xcorrraw的每个元素的时间延迟
[N,K]=size(V);  % K the number of mikes
r=8;  % increase in sampling rate采样率增加
N_line=r*N;
tau_line=fftshift(fft_base(N_line,dt/r));  % want large neg times first首先要大负数

% calculate the cross power spectrum for each pair, show计算每对的交叉功率谱，显示
n_pairs=K*(K-1)/2;
...
for i_pair=1:n_pairs
  ...
  % calc cross-power spectrum计算交叉功率谱
%   Xcorr_raw_this= ...
%     a(i_mike)*a(j_mike)*(V(:,i_mike).*conj(V(:,j_mike)));
  Xcorr_raw_this= ...
    V(:,i_mike).*conj(V(:,j_mike));

  % pad it, to increase resolution in time domain填充，以提高时域分辨率
  ...
  xcorr_raw_this=fftshift(real(ifft(Xcorr_raw_this_padded)));  
    % want large neg times first首先要大负数
